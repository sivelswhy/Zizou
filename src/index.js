const { Discord, Client, GuildManager, MessageEmbed, MessageActionRow, MessageButton, Permissions, InteractionWebhook, Guild, VoiceState
 } = require("discord.js");
...
const zizou = 'https://cdn.discordapp.com/attachments/965739781839523880/965929991797878834/ZIDANECHUT-NB__1_-removebg-preview.png'
      ...
      },
      {
        ...
        options: [
          ...
          },
          {
            type: 6,
            name: "utilisateur",
            description: "L'utilisateur à qui vous souhaitez envoyer ce message",
            required: true,
          },
          {
            name: 'direct',
            "type": 5,
            "description": "Envoyer le message directement à la personne sans décoration",
            "required": true
          },
        ],
      },
...
client.on("messageCreate", async (message) => {
  ...
  if (
    ...
    fs.writeFile('slashcommandperms.json', JSON.stringify(command), function (err) {
        if (err) throw err;
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.commandName === "weather") {
    ...
    await interaction.followUp({ files: ["https://cdn.discordapp.com/attachments/966300614093078578/966361155549401248/la-meteo-de-gulli.mp4"]})
      }
      });
client.on("interactionCreate", async (interaction) => {
  if (!interaction.isCommand()) return;
...
client.on('interactionCreate', interaction => {
  ...
  .setTitle("Messages Supprimés")
  .setDescription(`J'ai supprimé \`${number}\` messages dans le channel <#${interaction.channelId}> sous l'ordre de <@${interaction.user.id}>. `)  
  ...
  .setTitle(`<:enveloppe:966105701401387058> Message envoyé à un mec random`)
  .setDescription(`**<@${interaction.user.id}> a envoyé un message à ${userguy}. Voici son contenu :** \n\n${texte}\n\n`)
  .setTimestamp()
  .setThumbnail(zizou)
  .setFooter({ text: `${interaction.user.username}#${interaction.user.discriminator}`, icon_url: interaction.user.avatarURL() })
  .setColor("#82b597");
  if(direct === true){
      ...
      interaction.reply({content: `Message envoyé à ${userguy} contenant \`${texte}\` !`, ephemeral: true}).catch(error => { return interaction.reply({content: `Je ne peux pas envoyer le message à ${userguy}. J'ai reçu l'erreur ${error} !`, ephemeral: true}); });
      return client.channels.cache.get(logsmp).send({embeds: [logsembed]});
    } else {
  // console.log(userguy)
  const userembed = new MessageEmbed()
  .setTitle("<:enveloppe:966105701401387058> Vous avez reçu un message de l'administration")
  .setDescription(`**Vous avez reçu un message de l'administration. Voici son contenu :** \n\n${texte}`)
  .setColor("#82b597")
  .setThumbnail(zizou)
  .setFooter({ text: `${interaction.user.username}#${interaction.user.discriminator}`, icon_url: interaction.user.avatarURL() })
  .setTimestamp();
 userguy.send({embeds: [userembed]}).catch(error => { return interaction.reply({content: `Je ne peux pas envoyer le message à ${userguy}. J'ai reçu l'erreur ${error} !`, ephemeral: true}); });
  client.channels.cache.get(logsmp).send({embeds: [logsembed]});
  interaction.reply({content: `J'ai bien envoyé le message à ${userguy}. Tu peux retrouver un exemple de ce qu'il a reçu sous ce message`, embeds: [userembed], ephemeral: true});
  }
  }
  });
  ...
  client.on('messageCreate', async (message) => {
    ...
    if(message.channel.type === 'dm'){
      message.author.send(`Pour commencer une discussion avec le Staff, utilisez la commande ${prefix}start`)
    }
    if (message.content.startsWith(prefix + 'start')) { //If the message starts with prefix (!) and directly followed by start, continue.
      ...
      const DM = await message.author.send({content: `La discussion avec le staff a à présent commencé. Tout message que vous écrivez à partir de maintenant sera transmis à la direction.\n\nPour finir cette discussion, il vous suffira juste de taper !stop`}); //We're creating a DM channel with the user that ran the command.
      const startembed = new MessageEmbed()
      .setTitle(`Discussion commencé avec ${message.author.username}#${message.author.discriminator}`)
      .setDescription(`J'ai commencé une discussion privé avec ${message.author}. A partir de maintenant, tout les messages que cette personne envaira seront transmis dans ce channel`)
      .setColor('#3254a8')
      .setFooter({ text: `${message.author.username}#${message.author.discriminator}`, icon_url: message.author.avatarURL() })
      .setTimestamp()
      ...
      collector.on('collect', m => { //Triggered when the collector is receiving a new message
        ...
        .setDescription(`**Message reçu de la part de ${m.author} . Voici son contenu :** \n\n${m.content}\n\n`)
        .setTimestamp()
        .setThumbnail(zizou)
        .setFooter({ text: `${m.author.username}#${m.author.discriminator}`, icon_url: m.author.avatarURL() })
        .setColor("#82b597");
        if(m.attachments.size > 0){
          logsembed.setImage(m.attachments.first().proxyURL);
        }
        if(m.content === ''){
          logsembed.setDescription(`**Message reçu de la part de ${m.author} . Voici son contenu :** \n\n`)
        }
        client.channels.cache.get(logsmp).send({embeds: [logsembed]});
      })
      if(message.content.startsWith(prefix + 'stop')){
        collector.stop(`${message.author.username}#${message.author.discriminator}`);
        message.author.deleteDM()
      }
      collector.on('end', (collected) => { //Triggered when the collector ends. collected is a <Collection> of all messages collected and reason is the reason the collector ended.
        const endembed = new MessageEmbed()
        .setTitle(`Discussion terminé avec ${message.author.username}#${message.author.discriminator}`)
        .setDescription(`J'ai terminé la discussion privé avec ${message.author}. ${collected.size -1} messages ont été reçus.`)
        .setColor('#e8691a')
        .setFooter({ text: `${message.author.username}#${message.author.discriminator}`, icon_url: message.author.avatarURL() })
        .setTimestamp()
...
if (interaction.commandName === 'play') {
  if (!interaction.member.voice.channelId) return await interaction.reply({ content: "❌ | Vous n'êtes pas dans un salon vocal!", ephemeral: true });
        if (interaction.guild.me.voice.channelId && interaction.member.voice.channelId !== interaction.guild.me.voice.channelId) return await interaction.reply({ content: "❌ | Tu n'est pas dans le même salon vocal que le BOT !", ephemeral: true });
        ...
        try {
            ...
            queue.destroy();
            return await interaction.reply({ content: "❌ | Je n'ai pas pu rejoindre le salon vocal !", ephemeral: true });
        }
        ...
        if (!track) return await interaction.followUp({ content: `🔎❌ | le titre **\`${query}\`** n'a pas été trouvé`, ephemeral: true });

        queue.play(track);
        const playembed = new MessageEmbed()
        .setTitle(`Nouveau titre ajouté à la queue ♪`)
        .setDescription(`J'ai ajouté à la queue [**\`${track.title}\`**](${track.url})!\n\n`)
        .setThumbnail(track.thumbnail)
        .setFooter({ text: `${interaction.user.username}#${interaction.user.discriminator}`, icon_url: interaction.user.avatarURL() })
        .setColor("#82b597")
        .setTimestamp()

        .addFields({ name: "Auteur", value: `[${track.author}](${track.author.url})`, inline: true },
...
if (interaction.commandName === 'stop') {
  if (!interaction.member.voice.channelId) return await interaction.reply({ content: "Vous n'êtes pas dans un salon vocal!", ephemeral: true });
        ...
        queue.stop()
        return await interaction.reply({ content: "🚧 | La musique a été arrêtée !", ephemeral: false });
}
});
client.on('interactionCreate', async interaction => {
if (!interaction.isCommand()) return;
if (interaction.commandName === 'volume') {
  ...
  if(volumee > 100){
    ...
    return await interaction.reply({ content: "❌ | Le volume doit être compris entre 0 et 100 !", ephemeral: true });
    }
  }
  queue.setVolume(volumee)
  return await interaction.reply({ content: `🔉 | Le volume a été mis à **${volumee}%** !`, ephemeral: false });
}
});
client.on('interactionCreate', async interaction => {
if (!interaction.isCommand()) return;
...
// .setDescription(`${queue.createProgressBar()}\nVoici la queue du serveur :\n\n${queue.map(x => `**${x.track.title}** - [Lien](${x.track.url})`).join("\n")}`)
 .setDescription(`**Musique actuelle :** ${track}\n\n${queue.createProgressBar()}\n\n${queue}`)
.setFooter({ text: `${interaction.user.username}#${interaction.user.discriminator}`, icon_url: interaction.user.avatarURL() })
.setColor("#82b597")
.setTimestamp()
.setThumbnail(zizou)
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'skip') {
    ...
    if (!queue) return await interaction.reply({ content: "❌ | Il n'y a pas de musique en cours !", ephemeral: true });
    queue.skip();
    return await interaction.reply({ content: "⏭ | La musique a été skip !", ephemeral: false });
  }
});
let paused = false;
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'pause') {
    ...
    if(paused === false) {
      ...
      paused = true;
    return await interaction.reply({ content: "⏸ | La musique a été mise en pause !", ephemeral: false });
    } else if(paused === true) {
    queue.setPaused(false);
    paused = false;
    return await interaction.reply({ content: "▶️ | La musique a été reprise !", ephemeral: false });
    }
  }
});
client.on('interactionCreate', async interaction => {
...
if (interaction.commandName === '8ball') {
  ...
  .addFields({ name: "Question", value: `${capFL(question)}` }, { name: "Réponse", value:`${randomAnswer}` })
  .setFooter({ text: `${interaction.user.username}#${interaction.user.discriminator}`, icon_url: interaction.user.avatarURL() })
  .setColor("#82b597")
  .setTimestamp()
  .setThumbnail(zizou)
...
if (interaction.commandName === 'poll') {
  ...
  for (let i = 0; i < options.length; i++) {
    if (options !== "") {
      pollembed.addFields({ name: `Option ${i + 1}`, value: `${options[i]}` });
...
if (interaction.commandName === 'lyrics') {
  ...
  // }
  return interaction.reply({content: 'Cette commande est toujours en cours de développement, merci de réessayer plus tard.', ephemeral: true});
}
});
client.on('interactionCreate', async interaction => {
...
.setDescription(`J'ai sauvegardé le message suivant envoyé par ${msg.author}:\n\n\`${msg.content}\``)
.setColor('#82b597')
.setTimestamp()
if(msg.content === '') {
  embed.setDescription(`J'ai sauvegardé le message suivant envoyé par ${msg.author}:\n\n`)
}
if(msg.attachments.size > 0) {
  if(msg.attachments.first().proxyURL.endsWith('.png') || msg.attachments.first().proxyURL.endsWith('.jpg') || msg.attachments.first().proxyURL.endsWith('.jpeg')) {
  embed.setImage(msg.attachments.first().proxyURL)
  embed.setDescription(`J'ai sauvegardé le message suivant envoyé par ${msg.author}:\n\n`)
  } else {
    embed.setDescription(`Le message qu'à envoyé ${msg.author} contient un/des fichier(s) non supporté(s).`)
...
if (interaction.commandName === 'Info') {
  ...
  const embed = new MessageEmbed()
  .setAuthor({name: `${user.username}#${user.discriminator}`, icon_url: user.avatarURL()})
  .setTitle(`Information à propos de ${user.username}#${user.discriminator}`)
  .addFields({name: 'Indentifiant', value: `${user.id}`, inline: true},
  {name: 'Créé le', value: `<t:${Math.floor(user.createdTimestamp / 1000)}:f> (<t:${Math.floor(user.createdTimestamp / 1000)}:R>)`, inline: true},
  {name: 'Rôles', value: `${member.roles.cache.size > 0 ? member.roles.cache.map(r => r.name).join(',\n ') : 'Aucun'}`, inline: false},
  {name: 'Rejoint le', value: `<t:${Math.floor(member.joinedTimestamp / 1000)}:f> (<t:${Math.floor(member.joinedTimestamp / 1000)}:R>)`, inline: true},
  )
...
client.on('messageCreate', async message => {
  ...
  if(message.content.toLowerCase() === '!deploy2') {
    ...
    const role = message.guild.roles.cache.find(r => r.name === 'Ruby Nikara');
