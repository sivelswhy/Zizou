...
//   }
  ...
  client.on("ready", async () => {
    ...
    client.guilds.cache.forEach((guild) => {
      console.log(`${guild.name}  --- ${guild.id}, dont le fondateur est ${guild.ownerId}, \n`);
    });
    ...
    if (interaction.commandName === "weather") {
      ...
      const embed = new MessageEmbed()
        ...
        .addFields(
          ...
          {name: 'Température ressentie', value: `${Math.floor(res.main.feels_like)}°C`, inline: true},
          { name: 'Température minimum', value: `${Math.floor(res.main.temp_min)}°C`, inline: true },
          { name: 'Température maximal', value: `${Math.floor(res.main.temp_max)}°C`, inline: true },
    ...
    if (interaction.commandName === "infoserv") {
      ...
      const embed = new MessageEmbed()
        ...
        .addFields(
          ...
          { name: `Bloqué`, value: `${trueOrFalseInFR(res.info.locked)}`, inline: true },
          //   {
    ...
    if (interaction.commandName === "random") {
      if (interaction.options.getSubcommand() === "face") {
        ...
        const res = await axios(`https://this-person-does-not-exist.com/en?new`).catch(function (error) {
          console.warn(error);
          return interaction.reply({content: "<:warning:973943398178373663> | Une erreur est survenue lors de la génération d'un visage."})
        })
        if(!res) return;
        ...
        .setColor("#82b597")
        .setFooter({ text : `Nous tenons à rappeler que ce visage n'existe en aucun cas ! Pour plus d'informations, merci de visiter le WiKi.`, iconURL: `https://this-person-does-not-exist.com${res.data.src}`})
  ...
  client.on('interactionCreate', async interaction => {
    ...
    client.on('interactionCreate', interaction => {
      ...
      if (interaction.customId === 'support-modal') {
        ...
        .setDescription(`Nous avons bien reçu votre demande, à présent, voici un récapitulatif de votre demande :\n\n`)
        .addFields({name: 'Titre du problème', value: interaction.fields.getTextInputValue('problemTitle')},
        {name: 'Description du problème', value: interaction.fields.getTextInputValue('problem-description')})
        .setColor("#82b597")
        .setFooter({text: "Si votre demande est avéré à être un troll ou une demande faite uniquement pour occuper le temps du support, vous pourrez être ammené à être banni de l'utilisation de notre BOT"})
        const supportEmbed = new MessageEmbed()
        .setTitle('Nouveau ticket !')
        .setDescription(`Une nouvelle requête d'aide a été créer par **${interaction.user.username}#${interaction.user.discriminator}** *( ${interaction.user.id})*. Voici ci-dessous le contenu de son ticket.\n\n`)
        .addFields(
        {name: 'Titre du problème', value: interaction.fields.getTextInputValue('problemTitle')},
        {name: 'Description du problème', value: interaction.fields.getTextInputValue('problem-description')})
        .setColor("#82b597")
